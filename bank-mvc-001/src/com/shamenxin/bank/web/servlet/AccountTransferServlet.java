package com.shamenxin.bank.web.servlet;

import com.shamenxin.bank.exceptions.AppException;
import com.shamenxin.bank.exceptions.MoneyNotEnoughException;
...
/**
 * 在不使用MVC架构模式的前提下，完成银行账户的转账。
 * 分析这个程序存在那些问题？
 * 缺点1> 代码的复用性太差。（代码的重用性太差）
 * 导致缺点1的原因？
 *      因为没有进行”职能分工“，没有独立组件的概念，所以没有办法进行代码复用。代码和代码之间耦合度太高，扩展力太差。
 * 缺点2> 耦合度高，导致了代码很难扩展。
 * 缺点3> 操作数据库的代码和业务逻辑混杂在一起，很容易出错。
 *
 * 分析一下AccountTransferServlet它都负责了什么？
 * 1> 负责了数据的接收
 * 2> 负责了核心的业务逻辑
 * 3> 负责了数据库表中数据的CRUD操作（Create【增】 Retrieve【改】 Update【改】 Delete【删】）
 * 4> 负责了页面的数据展示。
 * 。。。。。
 *
 *
